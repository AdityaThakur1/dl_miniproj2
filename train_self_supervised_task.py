...
def main_worker(gpu, ngpus, args):
    ...
    train_losses = []
    train_acc1 , train_acc5 , val_losses, val_accuracy = [],[],[],[]
    for epoch in range(args.epochs):
        print("epoch ", epoch)
        train_sampler.set_epoch(epoch)

        train_loss , acc5, acc1 = train(train_loader, model, linear_classifier,
        ...
        with open(f'2104{args.task}_train_loss.npy', 'wb') as f:
            np.save(f, np.array(train_losses))
        with open(f'2104{args.task}_train_acc1.npy', 'wb') as f:
            np.save(f, np.array(train_acc1))
        with open(f'2104{args.task}_ train_acc5.npy', 'wb') as f:
            np.save(f, np.array( train_acc5))
        with open(f'2104{args.task}_val_losses.npy', 'wb') as f:
            np.save(f, np.array(val_losses))
        with open(f'2104{args.task}_val_accuracy.npy', 'wb') as f:
            np.save(f, np.array(val_accuracy))
...
def train(train_loader, model, linear_classifier, optimizer,
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if(best_acc1 < acc1.item()): 
            best_acc1 = acc1.item()
        if(best_acc5 < acc5.item()): 
            best_acc5 = acc5.item()
    ...
    return (train_loss, best_acc5, best_acc1)
